...
export default function RulesSection() {
  ...
  const rules = [
    ...
    },
    {
      title: '指标生政策',
      icon: Users,
      content: '符合指标生资格的考生可填报1个指标生志愿。指标生录取控制线为该校前三年（不含当年）第一批录取分数线的平均值下降20分。由于指标生批次先于第一批正取录取，且录取后后续12个正取志愿自动失效，因此指标生只能作为冲高机会位，不能当作保稳志愿。各公办普高未完成的指标生计划自动转为普通生计划。',
    },
    {
      title: '自主招生政策',
      icon: Star,
    ...
    },
    {
      title: '冲稳保填报策略',
      icon: Shield,
      content: '12个志愿的核心不是简单按学校名气堆叠，而是拉开梯度并把最好的可达学校尽量往前放。系统当前以“4冲4稳4保”为主结构，在中高分段会严格保持梯度；低分尾部则会优先补足公办学校密度，同时尽量避免学校过度扎堆在同一小分段。',
    },
  ];
