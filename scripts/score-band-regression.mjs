...
for (let score = startScore; score <= endScore; score += step) {
  ...
  if (
    score >= 490 &&
    (
      (strategyCounts['冲一冲'] || 0) !== 4 ||
      (strategyCounts['稳一稳'] || 0) !== 4 ||
      (strategyCounts['保一保'] || 0) !== 4
    )
  ) {
    failures.push(`${score}分: 490分以上未保持4冲4稳4保`);
  }

  if (score <= 480 && plan.items.length < 10) {
    failures.push(`${score}分: 低分段公办密度不足`);
  }
}
